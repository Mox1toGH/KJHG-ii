...
export const authApi = {
  ...
  login: (payload: LoginPayload) => apiClient.post<DetailResponse>('/accounts/login/', payload).then((r) => r.data),

  refresh: () => apiClient.post<DetailResponse>('/accounts/refresh/').then((r) => r.data),

  googleLogin: (payload: GoogleLoginPayload) =>
    apiClient.post<DetailResponse>('/accounts/google/', payload).then((r) => r.data),

  logout: () => apiClient.post<DetailResponse>('/accounts/logout/').then((r) => r.data),
  ...
  updateProfile: (payload: UpdateProfilePayload) => {
    ...
    return apiClient.patch<UserProfile>('/accounts/profile/', data).then((r) => r.data)
  },

  deleteAccount: (payload: DeleteAccountPayload) => apiClient.delete('/accounts/profile/', { data: payload }).then(() => undefined),

  getStatuses: () => apiClient.get<UserStatus[]>('/accounts/statuses/').then((r) => r.data),

  createStatus: (name: string) => apiClient.post<UserStatus>('/accounts/statuses/', { name }).then((r) => r.data),

  deleteStatus: (id: number) => apiClient.delete(`/accounts/statuses/${id}/`).then(() => undefined),
  ...
  addFriend: (payload: AddFriendPayload) => apiClient.post<Friend>('/accounts/friends/', payload).then((r) => r.data),

  removeFriend: (friendId: number) => apiClient.delete(`/accounts/friends/${friendId}/`).then(() => undefined),
